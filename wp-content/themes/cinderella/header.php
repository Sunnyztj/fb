...
<?php $stm_option = stm_glob_options(); ?>
<body <?php body_class(); ?>>
<div id="wrapper">
					...
					<div class="container">
						...
						<?php if( count( $top_bar_info ) > 1 ){ ?>
							<div class="top_bar_info_switcher">
								<div class="active"><?php echo esc_html( $top_bar_info[1]['office'] ); ?></div>
								<ul>
									<?php foreach( $top_bar_info as $key => $val ){ ?>
										<li><a href="#top_bar_info_<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $val['office'] ); ?></a></li>
									<?php } ?>
								</ul>
							</div>
						<?php } ?>
						<?php if( $top_bar_info ){
							foreach( $top_bar_info as $key => $val ){
								...
								<ul class="top_bar_info" id="top_bar_info_<?php echo esc_attr( $key ); ?>"<?php if( $key == 1 ){ echo ' style="display: block;"'; } ?>>
									<?php if( $val['address'] ){ ?>
										<li><i class="fa <?php echo esc_attr( $val['address_icon'] ); ?>"></i> <?php echo esc_html( $val['address'] ); ?></li>
									<?php } ?>
									<?php if( $val['phone'] ){ ?>
										<li><i class="fa <?php echo esc_attr( $val['phone_icon'] ); ?>"></i> <?php echo esc_html( $val['phone'] ); ?></li>
									<?php } ?>
									<?php if( $val['hours'] ){ ?>
										<li><i class="fa <?php echo esc_attr( $val['hours_icon'] ); ?>"></i> <?php echo esc_html( $val['hours'] ); ?></li>
									<?php } ?>
								</ul>
							<?php } ?>
						<?php } ?>
				...
				<div class="container">
					...
					<?php if( $header_hours = stm_option( 'working_hours' ) ){ ?>
						<div class="icon_text clearfix">
							...
							<div class="text">
								<?php echo nl2br( wp_kses_data( $header_hours ) ); ?>
							</div>
						</div>
					<?php } ?>
					<?php if( $header_address = stm_option( 'header_address' ) ){ ?>
						<div class="icon_text clearfix">
							...
							<div class="text">
								<?php echo nl2br( wp_kses_data( $header_address ) ); ?>
							</div>
						</div>
					<?php } ?>
				</div>
							...
							<div class="icon_text clearfix">
								...
								<div class="text">
									<?php if( $header_phone = stm_option( 'header_phone' ) ){ ?>
										<strong><?php echo esc_html( $header_phone ); ?></strong>
									<?php } ?>
