				...
				<?php foreach ( $val['images'] as $attach_id => $image ) { ?>
					...
					<div class="gallery all <?php echo esc_attr( $key ); ?>">
						<div class="gallery_wr">
							<?php echo stm_sanitize_text_field($image['thumbnail']); ?>
							<div class="overlay"></div>
							<h4><?php echo esc_html( $image['post_title'] ); ?></h4>

							<p><?php echo esc_html( $image['post_content'] ); ?></p>
							<a href="<?php echo esc_url( $image['p_img_large'][0] ); ?>"<?php echo stm_sanitize_text_field($pretty_rand); ?> class="prettyphoto"></a>
						</div>
					</div>
		...
		<script type="text/javascript">
			jQuery(document).ready(function ($) {
				...
				$(window).load(function () {
					...
					$(".gallery_grid_switcher").on('click', function () {
					...
					var $container_filter = $('#gallery_grid_filter_<?php echo esc_js( $id ); ?> ul a').on('click', function () {
