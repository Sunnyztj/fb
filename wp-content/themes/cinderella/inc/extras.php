...
if( ! function_exists( 'stm_comment' ) ){
	function stm_comment($comment, $args, $depth) {
		...
		<div class="comment-info clearfix">
			...
			<?php if( $rating ){ ?>
				<div itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating" class="star-rating" title="<?php echo sprintf( __( 'Rated %d out of 5', 'cinderella' ), $rating ) ?>">
					<span style="width:<?php echo ( esc_attr($rating) / 5 ) * 100; ?>%"><strong itemprop="ratingValue"><?php echo esc_html( $rating ); ?></strong> <?php _e( 'out of 5', 'cinderella' ); ?></span>
				</div>
			<?php } ?>
		...
		$fields    = array(
			...
			'url'    => '<div class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
						<div class="input-group comment-form-url">
							<input placeholder="' . esc_attr__( 'Website', 'cinderella' ) . '" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" />
						</div>
					</div></div>'
		);
	...
	function stm_wpml_lang_switcher() {
		if( function_exists( 'icl_get_languages' ) ){
			...
			if ( ! empty( $languages ) ) {
				...
				$output .= '</ul>';
				$output .= '</div>';
				echo stm_sanitize_text_field($output);
			}
		}
	}
}
...
function is_stm(){
	return get_option('is_stm') ? true : false;
}
